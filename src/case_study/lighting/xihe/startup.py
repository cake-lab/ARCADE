...
def main():
    xihenet = torch.jit.load("xihenet.pt")
    xihenet.eval()

    anchors_inf = xihenet_utils.fibonacci_sphere(samples=1280)
    anchors_xyz = torch.from_numpy(anchors_inf.T[np.newaxis, :, :])

    point_clouds = glob.glob("./inputs/*.npy")

    # Processing all the point clouds.
    for pcd_path in point_clouds:
        basename = pathlib.Path(pcd_path).name.split(".")[0]

        # Load Input Point Cloud.
        print(f"==> Loading input point cloud from {pcd_path}")
        point_cloud = np.load(pcd_path)
        xyz, rgb = point_cloud[:, :3], point_cloud[:, 3:]
        ...
        anchor_clr = np.zeros_like(anchors_inf) + 0.5
        anchor_clr[df.index] = rgb[df["index"]]

        print("Running inference.")
        anchor_clr = torch.from_numpy(anchor_clr.T[np.newaxis, :, :])
        p = xihenet(anchors_xyz, anchor_clr).detach()
        ...
        shc = u3d.SphericalHarmonics.from_array(
        ...
        shc_image = Image.fromarray(shc_image)

        print(f"Saving outputs for {basename}")
        # Save output SHC image.
        with open(f"./outputs/{basename}_shc.txt", "w") as f:
            f.write(json.dumps(coefficients_list))
        shc_image.save(f"./outputs/{basename}_envmap.png")
